...
if sys.platform != 'darwin':
  raise ImportError('This module is a darwin-specific implementation', name='pithy.darwin.clonefile', path=__file__)


libSystem = CDLL('libSystem.dylib', use_errno=True)
_clonefile = libSystem.clonefile # (UnsafePointer<Int8>!, UnsafePointer<Int8>!, UInt32) -> Int32
_clonefile.argtypes = (c_char_p, c_char_p, c_uint)
_clonefile.restype = c_int
...
def volume_supports_clone() -> bool:
  ...
  A volume can be tested for clonefile() support by using getattrlist(2) to get the volume capabilities attribute
