...
def raise_expected(*args:Any) -> NoReturn: raise Exception('expected')

utest_exc(Exception('expected'), raise_expected)

utest_seq([0, 1], range, 2)

def yield_then_raise(count:int) -> Any:
...
def raise_exc(arg:Any) -> NoReturn:
  raise Exception(arg)
