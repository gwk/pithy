...
'''
`craft-docs` is meant to build documentation trees using muck. It is currently outdated, because `muck` no longer uses a build directory.
'''
...
def main() -> None:
  ...
  for src in walk_files('_build/doc', file_exts=['.css', '.html', '.jpg', '.js', '.png', '.svg']):
    copy_path(src, replace_prefix(src, prefix='_build/doc', replacement='docs'), follow=True)
