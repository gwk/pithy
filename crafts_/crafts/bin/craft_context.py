...
Each imported file is appended as a block of the form:
```
Contents of ${path}:
...
Each `@path` token is kept but enclosed in a code span, which makes it inert.
Claude Code expands imports in any instructions file that it loads, including AGENTS.md.
A live token would load the imported file a second time, resolved against the directory of AGENTS.md instead of the importing file.
...
max_import_depth = 4 # Claude Code documents a maximum of four hops.
...
def expand_imports(src:str, text:str, *, header:str, out_dir:str, blocks:list[str], emitted:set[str], depth:int) -> None:
  ...
  for token, import_path in imports:
    ...
    header = f'Contents of {rel_path(path, start=out_dir)}:\n\n'
    expand_imports(path, text=dep_text, header=header, out_dir=out_dir, blocks=blocks, emitted=emitted, depth=depth+1)


def quote_imports(text:str, tokens:list[Token]) -> str:
  'Enclose each import token of `text` in a code span, so that no reader of the generated file treats it as an import.'
  parts:list[str] = []
  pos = 0
  for token in tokens:
    parts.append(f'{text[pos:token.pos]}`{text[token.pos:token.end]}`')
    pos = token.end
